...
현재 단계에서는 K-Means를 라이브러리 없이 구현하며, 문서를 cluster에 배정하는 과정을 학습합니다.
...
문서 vector들을 k개의 cluster로 나누기 위한 기본 구조를 구현합니다.

현재 구현된 단계:

- 초기 centroid 선택
- 각 문서를 가장 가까운 centroid에 배정

```text
Document
  |
  v
모든 Centroid와 similarity 계산
  |
  v
가장 가까운 Centroid 선택
  |
  v
해당 Cluster에 Document 배정
```

초기 centroid는 문서를 섞은 뒤 k개를 선택합니다.

```java
KMeans kMeans = new KMeans(3);
List<Centroid> centroids = kMeans.initialize(docs);

Map<Centroid, List<Document>> assignments =
        kMeans.assign(docs, centroids);
```
...
- `new KMeans(int k)`: k개의 cluster를 만드는 K-Means 객체 생성
- `initialize(List<Document> docs)`: 문서를 섞은 뒤 초기 centroid k개 선택
- `assign(List<Document> docs, List<Centroid> centroids)`: 문서를 가장 가까운 centroid에 배정
...
- `Centroid`: cluster의 대표 vector
- `Document`: id, text, vector, cluster를 가진 저장 단위
- `KMeans`: 초기 centroid 선택 및 문서 배정
...
5. K-Means 초기 centroid 선택과 문서 배정 - 진행 중
6. K-Means centroid 재계산과 반복 학습
7. IVF의 `nprobe` 아이디어 구현
8. ANN(Approximate Nearest Neighbor) 탐색 실험
9. HNSW 그래프 구조 구현
10. 탐색 과정 시각화

## Next Step

현재는 K-Means에서 초기 centroid를 선택하고, 각 문서를 가장 가까운 centroid에 배정할 수 있습니다.

다음 단계에서는 각 cluster에 배정된 문서들의 평균을 구해 centroid를 재계산하고, 이 과정을 반복하는 K-Means 학습 루프를 구현합니다.

이 과정을 이해하면 Vector DB가 index를 처음 만들 때 왜 많은 vector 연산을 수행하는지 알 수 있습니다.
